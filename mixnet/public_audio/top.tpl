	...
	if($('#query_audio').val() != 'Поиск по композициям и исполнителям' && $('#query_audio').val() != 0){
		...
		$.post('/index.php?go=public_audio&act=search', {query: $('#query_audio').val(), adres: '{adres}', pid: '{pid}'}, function(d){
			...
			if($('#seAudioNum').text() > 20){
				$('#seResult').html($('#seResult').html()+'<div id="audioAddedLoadAjax"></div><div class="cursor_pointer" style="margin-top:-4px" onClick="audioAddedLoadAjax()" id="wall_l_href_se_audio"><div class="public_wall_all_comm profile_hide_opne" id="wall_l_href_audio_se_load">Показать больше аудиозаписей</div></div>');
			}
...
<input type="text" value="Поиск по композициям и исполнителям" class="fave_input" id="query_audio" 
	...
	style="margin:0px;color:#c1cad0" 
maxlength="80" />
<div class="button_div fl_r"><button class="button" onClick="PublicAudioSearch(); return false" id="se_but_load">Найти</button></div>
<div class="clear"></div>
</div>
<div class="clear"></div>
<div id="seResult">
