...
var Albums = {
	...
	EditCover: function(id, page_num){
		...
		Box.Page(
			...
			lang_box_canсel, //�мя кнопки для закртие окна
...
var Photo = {
	...
	EditBox: function(id, r){
		...
		$.get('/index.php?go=albums&act=editphoto', {id: id}, function(data){
			Box.Show('edit_photo_'+id, '400', '� едактирование фотографии', '<div class="box_ppad"><div  style="color:#888;padding-bottom:5px;"><b>Описание фотографии</b></div><textarea class="inpst" id="descr_'+id+'" style="width:355px;height:71px;">'+data+'</textarea></div>', 'Отмена', 'Сохранить', 'Photo.SaveDescr('+id+', '+r+'); return false');
			Page.Loading('stop');
		});
	},
...
var messages = {
	new_: function(user_id){
		...
		'<div class="texta" style="width:100px">Тема:</div><input type="text" id="theme" class="inpst" maxlength="255" /><div class="mgclr"></div>'+
		'<div class="texta" style="width:100px">Сообщение:</div><textarea id="msg" class="inpst" style="height:120px;"></textarea><div class="mgclr"></div>'+
		'</div>';
...
var settings = {
			...
			if(new_pass != 0){
				if(new_pass2 != 0){
					...
					$.post('/index.php?go=settings&act=newpass', {old_pass: old_pass, new_pass: new_pass, new_pass2: new_pass2}, function(data){
						...
						butloading('saveNewPwd', 87, 'enabled', '�зменить пароль');
					});
				} else
		...
		if(name.length >= 2 && name != 0 && settings.isValidName(name)){
			if(lastname.length >= 2 && lastname != 0 && settings.isValidName(lastname)){
				...
				$.post('/index.php?go=settings&act=newname', {name: name, lastname: lastname}, function(data){
					$('.name_errors').hide();
					$('#ok_name').show();
					butloading('saveNewName', 69, 'enabled', '�зменить имя');
				});
			} else {
				$('.name_errors').hide();
				$('#err_name_1').show();
	...
	addblacklist: function(bad_user_id){
		...
		$.post('/index.php?go=settings&act=addblacklist', {bad_user_id: bad_user_id}, function(){
			...
			$('#text_add_blacklist').text('� азблокировать');
	...
	verification: function(){
		...
		if(skype != 0){
			if(docval != '/templates/Default/images/1.jpg' && docval2 != '/templates/Default/images/2.jpg'){
...
//GIFTS
var gifts = {
	box: function(user_id, c, cat){
		if(c)
			var cache = 0;
		else
			var cache = 1;

		Box.Page('/index.php?go=gifts&act=view', 'user_id='+user_id+'&cat='+cat, 'gifts', 679, lang_gifts_title, lang_box_canсel, 0, 0, 450, 1, 1, 1, 0, cache);
	},
...
var groups = {
	...
	savefeedback: function(id){
		...
		if($('#feedimg').attr('src') != template_dir+'/images/contact_info.png'){
			...
			$.post('/index.php?go=groups&act=addfeedback_db', {id: id, upage: upage, office: office, phone: phone, email: email}, function(d){
				if(d == 1){
					Box.Info('err', '�нформация', 'Этот пользователь уже есть в списке контактов.', 300, 2000);
	...
	saveinfo: function(id){
		...
		if(pattern.test(adres_page)){
			...
			$.post('/index.php?go=groups&act=saveinfo', {id: id, title: title, descr: descr, comments: comments, adres_page: adres_page, discussion: $('#discussion').val(), background_repeat: $('#background_repeat').val()}, function(d){
				if(d == 'err_adres')
	...
	editform: function(){
		...
		$('#public_editbg_container').animate({scrollLeft: "+650"});
	},
...
'</div>';
	...
	all_groups_user: function(for_user_id, page_num){
		...
		Box.Page('/index.php?go=groups&act=all_groups_user', 'for_user_id='+for_user_id+'&num='+num+page, 'all_groups_users_'+for_user_id+page_num, 525, '�нтересные страницы', lang_msg_close, 0, 0, 345, 1, 1, 1, 0, 1);
	}
}
...
var im = {
	...
	del: function(u){
		...
		$.post('/index.php?go=im&act=del', {im_user_id: u}, function(d){
			Box.Close('im_del'+u);
			Box.Info('ok_im', '�стория переписки удалена', 'Все сообщения диалога были успешно удалены.', 300, 3000);
			$('#okim'+u).remove();
		});
	},
...
var apps = {
	...
	mydel: function(i, t){
		if(t){
			$('.js_titleRemove').remove();
			$('#app'+i).html('<div align="center" style="color:#777;height:40px;padding-top:10px">�гра успешно удалена.</div>');
		} else
			$('#apps_rdel_txt').html('<div class="fl_r" style="color:#777;margin-top:6px;font-weight:normal">�гра удалена из списках ваших игр</div>');
...
var cover = {
	...
	del: function(){
		...
		$.post('/index.php?go=editprofile&act=delcover');
	},
	save: function(){
		cover.cancel();
		...
		s = t[1].split('px');
		s[0] = s[0].replace('-', '');
		$.post('/index.php?go=editprofile&act=savecoverpos', {pos: s[0]});
	},
	cancel: function(t){
		$('.cover_descring, .cover_addut').hide();
	...
	startedit: function(i, h){
		$('#upload_cover').show().text('�зменить фото');
