...
var messages = {
	new_: function(user_id){
		...
		'<div>Тема:</div><input type="text" id="theme" class="inpst"><div class="mgclr"></div>'+
		'<div>Сообщение:</div><textarea id="msg" class="inpst"></textarea><div class="mgclr"></div>'+
		'</div>';
...
var videos = {
	...
	load: function(){
		...
		video_lnk = video_lnk.replace("https", "http");
	...
	wall_send_comm: function(rec_id, public_id){
		...
		if(wall_text != 0){
			...
			$.post('/index.php?go=groups&act=wall_send_comm', {rec_id: rec_id, wall_text: wall_text, public_id: public_id, answer_comm_id: $('#answer_comm_id'+rec_id).val()}, function(data){
				...
				if(pattern.test(location.href)) $('#fast_text_'+rec_id+', #fast_inpt_'+rec_id).css();
			});
		} else
...
'</div>';
	...
	wall_video_add_box: function(){
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_videos', 400, 'Ссылка видеозаписи на УдинБала', '<div style="padding:15px;"><input  type="text"  placeholder="Введите ссылку видеозаписи на УдинБала.."  class="videos_input" id="video_attach_lnk" style="margin-top:10px" /></div>', lang_box_canсel, 'Прикрпепить', 'groups.wall_video_add_select()');
...
var rating = {
  ...
  update: function(){
	...
	if(add && new_rate >= 0){
	  ...
	  $('#num').text('аНаЕаДаОб?б?аАб?аОб?аНаО');
	  $('#rt').hide();
	} else {
	  $('#rt').show();
  ...
  page: function(){
    if($('#load_rate_prev_ubut').text() == 'а?аОаКаАаЗаАб?б? аПб?аЕаДб?аДб?б?аИаЕ аПаОаВб?б?аЕаНаИб?'){
	  textLoad('load_rate_prev_ubut');
	  $.post('/index.php?go=rating&act=view', {page_cnt: page_cnt_rate}, function(d){
		page_cnt_rate++;
		$('#rating_users').append(d);
		$('#load_rate_prev_ubut').text('а?аОаКаАаЗаАб?б? аПб?аЕаДб?аДб?б?аИаЕ аПаОаВб?б?аЕаНаИб?');
